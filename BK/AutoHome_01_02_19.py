 ...
 #       self.backup()
    ...
    def set_configs_user(self):
        try:
            ...
            cursor = conexao.cursor()
            # Executa o comando:
            cursor.execute("SELECT intensidade_iluminacao, temperatura, aprendizagem, economia, controle,ar_condicionado, televisao, temp_banho,temp_ar,modo_ar,volume_tv,canais_tv FROM configuracao_user  WHERE id=1")
            ...
            if cursor.rowcount > 0:
                ...
                if valor_televisao == True:
                    self.ui.btn_power_televisao.setText("Desligar")
                else:
                    self.ui.btn_power_tv.setText("Ligar")
    ...
    def machine_learn_acao(self):
        ...
        try:
            ...
            cursor = conexao.cursor()
            # Executa o comando:
            cursor.execute("SELECT acao,valor,horario FROM historico_acoes ORDER BY horario DESC")
            ...
            if cursor.rowcount > 0:
                ...
                for linha in resultado:
                    ...
                    data_string=linha[2].strftime('%H:%M:%S')
                    data_date=datetime.strptime(data_string,'%H:%M:%S')
                    #data=linha[2].strftime('%a')
                    mac_acao[qq][2]=data_date

                    #print(data_date + timedelta(minutes=15))
            ...
            for x in range(0,cursor.rowcount-1):
                if mac_acao[x][0] == 'Iluminacao':
                    ...
                    mac_power_tv[n_power_ar] = mac_acao[x]
            ...
            #for x in range(0, n_temperatura_ar):
             #   my_data = datetime.strptime(mac_temperatura_ar[x][2], '%H:%M:%S')

                #if mac_temperatura_ar[x + 1][2] > mac_temperatura_ar[x][2]+timedelta(minutes=15):
                #print(data_date+timedelta(minutes=15))
              #  print(my_data + timedelta(minutes=15))
